...
bool fprop_K128_N128(const float *I, const float *F, float *O,
  ...
  float *test_param;
  cudaError_t cuda_error;
  cuda_error = cudaMalloc((void**)&test_param, sizeof(float) * 2048);
  cudaMemset(test_param, 0, sizeof(float) * 2048);
  // arguments
  void *args[37] = {&test_param, &O, &I, &F, &alpha,
    &N, &K, &D, &H, &W, &WN, &HWN, &DHWN,
    &C, &KRST, &RST, &RS, &magic_RS, &shift_RS, &S, &magic_S, &shift_S,
  ...
  CUresult res = cuLaunchKernel(nervana_kernels[kernel_name],
    gridX, gridY, gridZ, 256, 1, 1, RST * 4 * 2 + 8, 0, args, NULL);
  if (res != CUDA_SUCCESS) {
    std::cerr << "Line " << __LINE__ << " error launching kernel " << kernel_name << " " << res << std::endl;
    return false;
  }
  cuCtxSynchronize();
  // output test_param
  // float* h_test = (float *)malloc(sizeof(float) * 2048);
  // for (int i = 0; i < 2048; ++i) {
  //   std::cout << h_test[i] << " ";
  // }
  // std::cout << std::endl;
  // cuda_error = cudaMemcpy(h_test, test_param, sizeof(float) * 2048, cudaMemcpyDeviceToHost);
  // if (cuda_error != cudaSuccess) {
  //   std::cerr << "Line " << __LINE__ << " memcpy error: " << cuda_error << std::endl;
  //   exit(1);
  // }
...
int main() {
  ...
  for (int i = 0; i < N; ++i) {
    for (size_t j = 0; j < K; ++j) {
      std::cout << "N: " << i << ", K: " << j << std::endl;
      for (size_t k = 0; k < M * P * Q; ++k) {
        std::cout << h_O[i * M * P * Q * K + j * M * P * Q + k] << " ";
      }
      std::cout << std::endl;
    }
    std::cout << std::endl;
